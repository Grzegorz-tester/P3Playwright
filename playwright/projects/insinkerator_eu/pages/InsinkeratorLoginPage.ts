...
export class InsinkeratorLoginPage extends LoginPage {
    ...
    readonly passwordInput = InsinkeratorObjects.LoginPage.passwordInput(this.page);
    readonly signInButton = InsinkeratorObjects.LoginPage.signInButton(this.page);
    readonly welcomeUserTopbarDiv = InsinkeratorObjects.LoginPage.welcomeUserTopbarDiv(this.page);
    readonly alertMessage = InsinkeratorObjects.LoginPage.alertMessage(this.page);
    readonly forgotPasswordLink = InsinkeratorObjects.LoginPage.forgotPasswordLink(this.page);
    readonly signOutLink = InsinkeratorObjects.LoginPage.signOutLink(this.page);
    ...
    private async fillAndVerify(locator: ReturnType<Page['locator']>, value: string, maxAttempts: number = 4): Promise<void> {
        ...
        throw new Error(`Field value did not stick after ${maxAttempts} attempts (expected "${value}", got "${await locator.inputValue()}"). This likely indicates the login form is remounting — see fillAndVerify note in InsinkeratorLoginPage.ts.`)
    }
}
