...
export class InsinkeratorEuProductListPage extends AbstractProductListPage {

    constructor(page: Page) {
        super(page);
    }

    readonly productNameLink = InsinkeratorEuObjects.ProductListPage.productNameLink(this.page);
    readonly searchHitsHeading = InsinkeratorEuObjects.SearchResultsPage.hitsHeading(this.page);
    readonly searchHitCount = InsinkeratorEuObjects.SearchResultsPage.hitCount(this.page);
    readonly searchResultProductNameLink = InsinkeratorEuObjects.SearchResultsPage.productNameLink(this.page);
    readonly hitsHeading = InsinkeratorEuObjects.ProductListPage.hitsHeading(this.page);
    readonly hitCount = InsinkeratorEuObjects.ProductListPage.hitCount(this.page);
    readonly productCardLink = InsinkeratorEuObjects.ProductListPage.productCardLink(this.page);
    readonly productCardPrice = InsinkeratorEuObjects.ProductListPage.productCardPrice(this.page);
    readonly filterAndSortOpenButton = InsinkeratorEuObjects.ProductListPage.filterAndSortOpenButton(this.page);
    readonly facetCheckboxes = InsinkeratorEuObjects.ProductListPage.facetCheckboxes(this.page);
    readonly sortByOptions = InsinkeratorEuObjects.ProductListPage.sortByOptions(this.page);
    readonly loadMoreButton = InsinkeratorEuObjects.ProductListPage.loadMoreButton(this.page);
    readonly currentItemsCount = InsinkeratorEuObjects.ProductListPage.currentItemsCount(this.page);
    readonly totalItemsCount = InsinkeratorEuObjects.ProductListPage.totalItemsCount(this.page);

    // VERIFIED — reached via /search?q=<term> (see
    // InsinkeratorEuHomePage.submitSearchFromDrawer). Confirmed live that
    ...
    // InsinkeratorEuPDPage.validateProductNameMatches).
    ...
    // InsinkeratorEuHomePage.chooseMenuCategory()) was fixed, so its two
    ...
    async clickOnAProductToProceedToPDP(productName: string): Promise<void> {
        const productLink = InsinkeratorEuObjects.ProductListPage.productNameLinkFiltered(productName)(this.page)
