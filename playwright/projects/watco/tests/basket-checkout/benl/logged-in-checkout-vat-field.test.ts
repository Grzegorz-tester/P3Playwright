...
/**
 * WAT-305 — VAT registration number field, logged-in checkout (BE-NL).
 * Mirrors logged-in-checkout-vat-field-nl.test.ts — same VAT-gated Pay on
 * Account behaviour (see guest-checkout-vat-field-benl.test.ts docblock).
 */
...
test('BE-NL logged-in checkout, no saved VAT: field is empty and Pay on Account is hidden', async ({ page, loginPage, homePage, productListPage, productDetailPage, basketPage, checkoutPage }) => {
    await test.step('Log in with an account that has no saved VAT number', async () => {
        ...
        await loginPage.loginToApplication(watcoBeNl.accountTestUser_1.email, watcoBeNl.accountTestUser_1.password, '/mijn-account')
    })
...
test('BE-NL logged-in checkout, has saved VAT: field is pre-populated, Pay on Account is visible on load, and both are editable', async ({ page, loginPage, homePage, productListPage, productDetailPage, basketPage, checkoutPage }) => {
    ...
    await test.step('VAT field is pre-populated and Pay on Account is already visible, with no Apply click needed', async () => {
        ...
        await expect(checkoutPage.vatNumberInput).toHaveValue('BE0123456749')
        await expect(checkoutPage.vatApplyButton).toBeDisabled()
    ...
    await test.step('Editing and applying a new VAT number updates the field', async () => {
        ...
        await checkoutPage.applyVatNumber('BE0987654321')
        await expect(checkoutPage.vatNumberInput).not.toHaveClass(/is-invalid/)
        await expect(checkoutPage.vatNumberInput).toHaveValue('BE0987654321')
    })
})

test('BE-NL logged-in checkout, has saved VAT: clearing and applying persists an empty value and hides Pay on Account again', async ({ page, loginPage, homePage, productListPage, productDetailPage, basketPage, checkoutPage }) => {
    await test.step('Log in with an account that has a saved VAT number', async () => {
        console.log('[STEP] Log in with an account that has a saved VAT number')
        await loginPage.navigateToLoginPage('/inloggen')
        await dismissCookieBanner(page)
