...
/**
 * WAT-305 — VAT registration number field, Express Checkout (BE-FR).
 * Mirrors express-checkout-vat-field-fr.test.ts — same deferred scope
 * (real wallet-sheet interaction) applies here too.
 */
test('BE-FR express checkout: VAT number field', async ({ page, homePage, productListPage, productDetailPage, basketPage, checkoutPage }) => {
    ...
    await test.step('A valid VAT number is applied cleanly', async () => {
        ...
        await checkoutPage.applyExpressVatNumber('BE0411905847')
        await expect(checkoutPage.expressVatNumberInput).not.toHaveClass(/is-invalid/)
        await expect(checkoutPage.expressVatNumberInput).toHaveValue('BE0411905847')
    })
})
