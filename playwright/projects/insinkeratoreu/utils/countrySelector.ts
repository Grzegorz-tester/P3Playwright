...
import { InsinkeratorEuObjects } from './objects';
...
export async function selectCountryOnFreshLoad(page: Page, countryName: string): Promise<void> {
    const overlay = InsinkeratorEuObjects.CountrySelector.blockingOverlay(page);
    ...
    // (InsinkeratorEuCheckoutPage.chooseDeliveryAddress) — same class of bug.
    const overlayAppeared = await overlay
        .waitFor({ state: 'visible', timeout: 8000 })
        .then(() => true)
        .catch(() => false)
    if (overlayAppeared) {
        const option = InsinkeratorEuObjects.CountrySelector.countryModalOption(countryName)(page);
...
export async function changeCountry(page: Page, countryName: string): Promise<void> {
    await InsinkeratorEuObjects.CountrySelector.utilityBarOpenButton(page).click();
    const option = InsinkeratorEuObjects.CountrySelector.countryOption(countryName)(page);
    await expect(option).toBeVisible({ timeout: 15000 });
    await option.click();
}
