import {test as setup, expect} from '@playwright/test';
import {carbon} from '@utils/testUsers'
import {testConfig} from '@utils/testConfig';

function delay(ms: number) {
    return new Promise( resolve => setTimeout(resolve, ms) );
}

const accountTestUser_1_file = (process.env.CI ? process.env.CI_PROJECT_DIR + '/playwright/' : '') + `${process.env.PROJECT}/tests/.auth/accountTestUser_1.json`;
setup.skip('authenticate as user 1', async ({ request}) => {
    const loginResponse = await request.post(`${testConfig.getApi(process.env.PROJECT)}/auth`, {
        data: {
            'email': carbon.testUser_1.email,
            'password': carbon.testUser_1.password
        }, timeout: 20000
    });
    await delay(2000)
    await expect(loginResponse).toBeOK();
    await request.storageState({ path: accountTestUser_1_file });
});

const accountTestUser_2_file = (process.env.CI ? process.env.CI_PROJECT_DIR + '/playwright/' : '') + `${process.env.PROJECT}/tests/.auth/accountTestUser_2.json`;
setup.skip('authenticate as user 2', async ({ request}) => {
    const loginResponse = await request.post(`${testConfig.getApi(process.env.PROJECT)}/auth`, {
        data: {
            'email': carbon.testUser_2.email,
            'password': carbon.testUser_2.password
        }, timeout: 20000
    });
    await delay(2000)
    await expect(loginResponse).toBeOK();
    await request.storageState({ path: accountTestUser_2_file });
});


